...
def add():
    ...
    if name == None:
        return Response('{"result": false, "error": 1, "description": "Cannot proceed because you did not provide the name of the apartment you want to reserve."}', status=400, mimetype="application/json")
    ...
    for reservation in reservations:
        ...
        for i in range(int_duration + 1):
            ...
            if str(string_date) == str(string_date_new):
                return Response('{"result": false, "error": 4, "description": "Cannot proceed because this apartment is already reserved in that timeperiod"}', status=400, mimetype="application/json")
...
def apartment_added(ch, method, properties, body):
    ...
    connection = sqlite3.connect("/home/data/reservations.db", isolation_level=None)
    cursor = connection.cursor()
    cursor.execute("INSERT INTO apartments VALUES (?, ?, ?)", (id, name, size))
    cursor.close()
    connection.close()

def apartment_removed(ch, method, properties, body):
    data = json.loads(body)
    id = data["id"]
    name = data["name"]

    logging.info(f"Removing apartment {name}...")
...
def deregister(): 
    connection = consul.Consul(host='consul', port=8500)
    connection.agent.service.deregister("reserve", address="reserve", port=5000)
...
if __name__ == "__main__":
    ...
    channel.basic_consume(queue=queue_name, on_message_callback=apartment_added, auto_ack=True)
    logging.info("Waiting for messages.")

    result = channel.queue_declare(queue="", exclusive=True)
    queue_name = result.method.queue
    channel.queue_bind(exchange="apartments", queue=queue_name, routing_key="removed")
    channel.basic_consume(queue=queue_name, on_message_callback=apartment_removed, auto_ack=True)
    logging.info("Waiting for messages.")
    ...
    if os.path.exists("/home/data/reservations.db"):
        ...
        if address is not None and port is not None:
            response = requests.get(f"http://{address}:{port}/apartments")
            data = response.json()

            logging.info("Data received: " + data)

            for entry in data["apartments"]:
                cursor.execute("INSERT INTO apartments VALUES (?, ?, ?)", (entry["id"], entry["name"], entry["squaremeters"]))
