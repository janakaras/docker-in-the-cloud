...
def search():
    ...
    cursor.execute("SELECT DISTINCT name, squaremeters FROM apartments")
        ...
        for reservation in reservations:
            ...
            for i in range(int_duration + 1):
                ...
                if str(string_date) == str(string_date_new):
                    exclude_apartments.append(name)
                    break
    ...
    for apartment in apartments:
        if apartment[0] in exclude_apartments:
    ...
    for apartment in apartments:
        results += f"<p> {apartment[0]}, {apartment[1]}</p>\n"
...
def deregister(): 
    connection = consul.Consul(host='consul', port=8500)
    connection.agent.service.deregister("search", address="search", port=5000)
